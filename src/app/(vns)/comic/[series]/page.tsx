...
export default function ComicSeriesDetail(properties: PageProps<"/comic/[series]">) {
    ...
    return (
        <>
            <ArticleJsonLd
                headline={data.title}
                datePublished={data.createdAt ? new Date(data.createdAt).toISOString() : ""}
                dateModified={data.updatedAt ? new Date(data.updatedAt).toISOString() : ""}
                author="Publisher: @terrastationvn"
                            ...
                            <div className="place-items-center-safe mt-6 flex w-full flex-col md:flex-row">
                                ...
                                <div className="flex w-full flex-col place-content-between gap-4 px-4 md:w-2/3">
                                    ...
                                    <Button
                                        ...
                                        render={
                                            <Link
                                                href={`/comic/${series}/${data.chapters[0].comicChapterId}` as Route}
                                            />
                                        }
                                        nativeButton={false}
                                    >
