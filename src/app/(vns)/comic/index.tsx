...
export const Route = createFileRoute("/(vns)/comic/")({
  loader: ({ context }) => {
    context.queryClient.ensureInfiniteQueryData(comicSeriesListingQueryOptions());
  },
  head: () => ({
    meta: [{ title: "Arknights VNS | Truyện tại Trạm" }],
  }),
  component: ComicListing,
  ssr: "data-only",
});
              ...
              return (
                <div className="place-items-center-safe flex flex-col gap-2" key={comic.comicSeriesId}>
                  ...
                  <Link
                    ...
                    params={{ series: comic.comicSeriesId }}
                    to="/comic/$series"
                  >
                    {comic.title}
                  </Link>
