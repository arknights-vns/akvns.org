    ...
    return (
        <div>
            <div className={"relative max-w-[900px] w-full mx-auto my-[60px] px-0 py-[20px] flex justify-center items-center"}>
                <div className={"relative w-[1000px] h-1 bg-[linear-gradient(to_right,_lightgray,_darkgray,_gray,_gray,_darkgray,_lightgray)] rounded-[4px]"}>
                    {visibleItems.map((item, index_) => {
                        const leftPercent = (index_ + ((visibleItems.length - 1) / 2 - index_) * 0.1) / (visibleItems.length - 1) * 100;
                        return (
                            <div
                                className={"absolute top-2/4 -translate-y-1/2 w-[18px] h-[18px] bg-[#fff] border-[3px] border-[#000] rounded-[50%] cursor-pointer"}
                                key={item.name}
                                style={{ left: `${leftPercent}%` }}
                            >
                                <div className={"absolute top-[28px] left-2/4 -translate-x-1/2 text-[14px] font-semibold whitespace-nowrap"}>{item.date}</div>
                                <div className={"relative top-[50px] left-1/2 -translate-x-1/2 min-w-[180px] min-h-[200px] p-3 bg-background border border-[#ccc] rounded-[8px] text-center [box-shadow:0_0_8px_rgba(0, 0, 0, 0.15)] block z-10"}>
